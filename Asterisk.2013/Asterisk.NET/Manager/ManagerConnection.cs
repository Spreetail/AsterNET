	...
	public class ManagerConnection
	{
		...
		protected internal bool connect()
		{
			...
			lock (lockSocket)
			{
				if (mrSocket == null)
				{
					logger.InfoFormat("Connecting to {0}:{1}", hostname, port);
					try
					{
		...
		{
			logger.WarnFormat("reconnect (init: {0}), reconnectCount:{1}", init, reconnectCount);
					...
					else
					{
						try
						{
							if (retryCount < reconnectRetryFast)
							{
								...
								logger.InfoFormat("Reconnect delay : {0}, retry : {1}", reconnectIntervalFast, retryCount);
							...
							else
							{
								...
								logger.InfoFormat("Reconnect delay : {0}, retry : {1}", reconnectIntervalMax, retryCount);
						...
						{
							logger.Info("Reconnect delay exception : ", ex);
							continue;
						}
		...
		internal void SendToAsterisk(ManagerAction action, string internalActionId)
		{
			...
			logger.DebugFormat("Sent action : '{0}' : {1}", internalActionId, action);
			if (sa == null)
		...
		{
			logger.DebugFormat("Dispatch response packet : {0}", Helper.JoinVariables(buffer, ", ", ": "));
			DispatchResponse(buffer, null);
		}

		internal void DispatchResponse(ManagerResponse response)
		{
			logger.DebugFormat("Dispatch response : {0}", response);
		...
		internal void DispatchResponse(Dictionary<string, string> buffer, ManagerResponse response)
		{
			...
			if (!string.IsNullOrEmpty(actionId))
			{
				...
				if (responseHandler != null)
				{
					...
					{
						logger.ErrorFormat("Unexpected exception in responseHandler {0}\n{1}", response, ex);
		...
		{
			logger.DebugFormat("Dispatching event: {0}", e);
			...
			if (e is ConnectEvent)
			{
				...
				logger.InfoFormat("Connected via {0}", protocol);
				...
				else
				{
					...
					logger.WarnFormat("Unsupported protocol version '{0}'. Use at your own risk.", protocol);
				}
				if (reconnected)
				{
